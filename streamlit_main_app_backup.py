...
with tab_kg:
    try:
        ...
        if not neo4j_enabled: 
            ...
            if st.session_state.get("selected_study"):
            ...
            st.error(f"Knowledge Graph tab encountered an error: {str(e)}")
            st.info("This tab requires Neo4j connection and proper session state. Please check your configuration.")
            
            # Provide fallback functionality
            if st.session_state.get('selected_study_for_kg'):
                st.info(f"Selected study: {st.session_state.selected_study_for_kg}")
                if st.button("Clear Selection", key="clear_kg_fallback"):
